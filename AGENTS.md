...
- **Execution Boundaries:** NEVER run commands to test the application, start/stop background services, or run simulated workloads (e.g., `uv run fastapi dev`, `npm run dev`, `generator.py`). Provide the necessary commands clearly in markdown blocks and leave the execution strictly to the user.

## Python Development
- **Use uv:** Always use `uv` for package management and running scripts (e.g., `uv add`, `uv run`, `uv sync`). Avoid using pip directly.
